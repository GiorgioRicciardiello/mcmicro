...
# Create Apptainer cache dir on scratch (not home — small quota)
mkdir -p /sc/arion/scratch/giocrm/.apptainer_cache
mkdir -p /sc/arion/scratch/giocrm/nf-work
mkdir -p /sc/arion/scratch/giocrm/.nextflow
...
MCMICRO environment loaded:
  ...
  NXF_WORK  : /sc/arion/scratch/giocrm/nf-work
  CACHE     : /sc/arion/scratch/giocrm/.apptainer_cache
```
...
- All containers are cached to `/sc/arion/scratch/giocrm/.apptainer_cache` (reused in future runs)
...
find /sc/arion/scratch/giocrm/nf-work -name ".command.log" | xargs tail -f
...
cat /sc/arion/scratch/giocrm/nf-work/xx/yyyyyyyy/.command.err
```
...
echo $NXF_WORK   # must be /sc/arion/scratch/giocrm/nf-work
source mcmicro_env.sh
bash run_FNEL03.sh
```
